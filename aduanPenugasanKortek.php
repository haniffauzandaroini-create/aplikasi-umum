    ...
    <div class="wrapper" style="min-height: 223.667px;">
      ...
      <div class="content-wrapper">
        ...
        <?php include 'pagination.php';
          ...
          $reload = "aduanPenugasanKortek.php?pagination=true&keyword=$keyword";
          ...
          WHERE (r.id LIKE '%$keyword%' OR r.kode_tiket LIKE '%$keyword%' OR r.tgl_pengajuan LIKE '%$keyword%' OR r.tgl_penugasan LIKE '%$keyword%' OR r.tgl_pra_penanganan LIKE '%$keyword%' OR r.tgl_proses_penanganan LIKE '%$keyword%' OR r.tgl_selesai_penanganan LIKE '%$keyword%' OR r.deskripsi_kerusakan LIKE '%$keyword%' OR dau_admin.username LIKE '%$keyword%' OR dau_admin.nm_person LIKE '%$keyword%' OR dau_admin.domain_person LIKE '%$keyword%' OR dau_koord.username LIKE '%$keyword%' OR dau_koord.nm_person LIKE '%$keyword%' OR dau_koord.domain_person LIKE '%$keyword%' OR dau_pelaksana.username LIKE '%$keyword%' OR dau_pelaksana.nm_person LIKE '%$keyword%' OR dau_pelaksana.domain_person LIKE '%$keyword%' OR olp.nm LIKE '%$keyword%' OR dau.username LIKE '%$keyword%' OR dau.nm_person LIKE '%$keyword%' OR dau.domain_person LIKE '%$keyword%' OR olk.nm_kampus LIKE '%$keyword%' OR olg.nm_gedung LIKE '%$keyword%' OR olr.nm_ruang LIKE '%$keyword%' OR oak.nm LIKE '%$keyword%' OR otk.nm LIKE '%$keyword%' OR oap.nm LIKE '%$keyword%' OR ov.nm LIKE '%$keyword%') AND r.kat_koordinator='4' AND r.status_tiket ='2' ORDER BY r.id DESC";
          
          $result = mysqli_query($con, $sql);
          }else{
          $reload = "aduanPenugasanKortek.php?pagination=true";
          $sql = "SELECT * FROM tiket_masuk WHERE kat_koordinator='4' AND status_tiket ='2' ORDER BY tgl_pengajuan DESC";
          $result = mysqli_query($con, $sql);
          }
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm mb-2">
                <form method="post" action="aduanPenugasanKortek.php">
                  ...
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      ...
                      <?php
                        if($_REQUEST['keyword']<>""){
                        ?>
                      <a class="btn btn-sm btn-warning" title="Refresh" href="aduanPenugasanKortek.php"><i class="fas fa-sync"></i> Refresh</a>
                      <?php
                        }
                        ?>
                    </div>
                      ...
                      <table class="table table-hover m-0 table-sm custom">
                        ...
                        <tbody>
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <td class="">
                              <form action="updateStatusAduanPenugasanKortek.php" method="post">
                                <input type="text" name="id" class="sr-only" value="<?php echo $data['id'];?>" required readonly>
                                <input type="text" name="page" class="sr-only" value="<?php echo $page;?>" required readonly>
                                <input type="text" name="nm_koordinator" class="sr-only" value="<?php echo $dtUser['username'];?>" required readonly>
                                <select name='status_tiket' class='form-control form-control-xs' onchange='this.form.submit();' style="width: 120px;" required>
                                <?php
                                  $tampil = mysqli_query($con,  "SELECT * FROM opsi_status_tiket WHERE id IN (2,3) ORDER BY id ASC" );
