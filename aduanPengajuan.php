    ...
    <div class="wrapper" style="min-height: 223.667px;">
      ...
      <div class="content-wrapper">
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item active">Aduan Masuk</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <?php include 'pagination.php';
          ...
          $reload = "aduanPengajuan.php?pagination=true&keyword=$keyword";
          ...
          WHERE (r.id LIKE '%$keyword%' OR r.kode_tiket LIKE '%$keyword%' OR r.tgl_pengajuan LIKE '%$keyword%' OR r.tgl_penugasan LIKE '%$keyword%' OR r.tgl_pra_penanganan LIKE '%$keyword%' OR r.tgl_proses_penanganan LIKE '%$keyword%' OR r.tgl_selesai_penanganan LIKE '%$keyword%' OR r.deskripsi_kerusakan LIKE '%$keyword%' OR dau_admin.username LIKE '%$keyword%' OR dau_admin.nm_person LIKE '%$keyword%' OR dau_admin.domain_person LIKE '%$keyword%' OR dau_koord.username LIKE '%$keyword%' OR dau_koord.nm_person LIKE '%$keyword%' OR dau_koord.domain_person LIKE '%$keyword%' OR dau_pelaksana.username LIKE '%$keyword%' OR dau_pelaksana.nm_person LIKE '%$keyword%' OR dau_pelaksana.domain_person LIKE '%$keyword%' OR olp.nm LIKE '%$keyword%' OR dau.username LIKE '%$keyword%' OR dau.nm_person LIKE '%$keyword%' OR dau.domain_person LIKE '%$keyword%' OR olk.nm_kampus LIKE '%$keyword%' OR olg.nm_gedung LIKE '%$keyword%' OR olr.nm_ruang LIKE '%$keyword%' OR oak.nm LIKE '%$keyword%' OR otk.nm LIKE '%$keyword%' OR oap.nm LIKE '%$keyword%' OR ov.nm LIKE '%$keyword%') AND r.status_tiket IN (1,10) ORDER BY r.id DESC";
          
          $result = mysqli_query($con, $sql);
          }else{
          $reload = "aduanPengajuan.php?pagination=true";
          $sql = "SELECT * FROM tiket_masuk WHERE status_tiket IN (1,10) ORDER BY id DESC";
          $result = mysqli_query($con, $sql);
          }
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-12 col-md-9 col-lg-9 mb-2">
                <form method="post" action="aduanPengajuan.php">
                  ...
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      ...
                      <?php
                        if($_REQUEST['keyword']<>""){
                        ?>
                      <a class="btn btn-sm btn-warning" title="Refresh" href="aduanPengajuan.php"><i class="fas fa-sync"></i> Refresh</a>
                      <?php
                        }
                        ?>
                    </div>
              ...
              <div class="col-sm-12 col-md-3 col-lg-3 mb-2">
                <a type="button" class="btn btn-info btn-sm btn-block" href="inputTiketBaruAdm.php?page=<?php echo $page;?>"><i class="fas fa-cart-plus"></i> Ajukan Aduan Baru</a>
              </div>
            </div>
            <div class="row">
              <section class="col-sm-12 connectedSortable">
                <div class="card card-outline card-info">
                  <div class="card-header">
                    <div class="clearfix">
                      <h4 class="card-title float-left">Aduan Masuk</h4>
                    </div>
                  </div>
                    ...
                    <div class="table-responsive-no-wrap">
                      <table class="table table-hover m-0 table-sm custom">
                        <thead class="thead-light">
                          ...
                          <th class="text-center" width="4%"><i class="fas fa-comment-alt"></i></th>
                        ...
                        <tbody>
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <td class="">
                              <form action="updateStatusPengajuanTiket.php" method="post">
                                <input type="text" name="id" class="sr-only" value="<?php echo $data['id'];?>" required readonly>
                                <input type="text" name="page" class="sr-only" value="<?php echo $page;?>" required readonly>
                                <input type="text" name="admin" class="sr-only" value="<?php echo $dtUser['username'];?>" required readonly>
                                <select name='status_tiket' class='form-control form-control-xs' onchange='this.form.submit();' style="width: 100px;" required>
                                <?php
                                  $tampil = mysqli_query($con,  "SELECT * FROM opsi_status_tiket WHERE id IN (1,2,10) ORDER BY id ASC" );
                                  while ( $w = mysqli_fetch_array( $tampil ) ) {
                                  if ( $data['status_tiket'] == $w[ 'id' ] ) {
                            ...
                            <td class=""><a class="btn btn-info btn-xs btn-block" href="catatanPengajuanTiket.php?id=<?php echo $data['id'];?>&page=<?php echo $page;?>" title="Beri catatan"><i class="far fa-comment-alt"></i></a></td>
                            <td class=""><a class="btn btn-danger btn-xs btn-block" href="deleteTiketMasukAdm.php?id=<?php echo $data['id'];?>&page=<?php echo $page;?>" onclick="return confirm(\"Yakin data ini dihapus")" title="Hapus data"><i class="far fa-trash-alt"></i></a></td>
